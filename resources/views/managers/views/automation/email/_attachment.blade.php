<form action="{{ route('Automation2Controller@emailAttachmentUpload', [
  'uid' => $automation->uid,
  'email_uid' => $email->uid,
]) }}" class="dropzone">
  {{ csrf_field() }}
            ...
            <ul class="key-value-list list-small">
                @foreach ($email->attachments as $attachment)
                    <li class="flex align-items-center">
                        ...
                        <div class="action">
                            <a
                                href="{{ route('Automation2Controller@emailAttachmentDownload', [
                                    'uid' => $automation->uid,
                                    'email_uid' => $email->uid,
                            ...
                            <a
                                href="{{ route('Automation2Controller@emailAttachmentRemove', [
                                    'uid' => $automation->uid,
                                    'email_uid' => $email->uid,
                                    'attachment_uid' => $attachment->uid,
                                ]) }}"
