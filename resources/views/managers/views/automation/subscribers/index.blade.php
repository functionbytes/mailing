...
@section('page_header')
	<main class="container px-3 pt-4">
		<ul class="breadcrumb breadcrumb-caret position-right">
			...
			<li class="breadcrumb-item"><a href="{{ action("manager.automations") }}">{{ trans('messages.automations') }}</a></li>
...
@section('content')
	<main class="container px-3">
		<div class="listing-form"
			data-url="{{ route('Automation2Controller@subscribersList', $automation->uid) }}"
						...
						<span class="me-2">
							...
							<div class="btn-group list_actions me-2" style="display:none">
								...
								<ul class="dropdown-menu">
									<li>
										<a class="dropdown-item"  link-method="POST" link-confirm="{{ trans('messages.subscribe_subscribers_confirm') }}" href="{{ route('SubscriberController@subscribe', $list->uid) }}">
											<span class="material-symbols-rounded">mark_email_read</span> {{ trans('messages.subscribe') }}
										</a>
									</li>
									<li>
										<a class="dropdown-item" link-method="POST" link-confirm="{{ trans('messages.unsubscribe_subscribers_confirm') }}" href="{{ route('SubscriberController@unsubscribe', $list->uid) }}">
											<span class="material-symbols-rounded">logout</span> {{ trans('messages.unsubscribe') }}
										</a>
									</li>
									<li>
										<a class="dropdown-item list-action-single" link-method="POST" link-confirm="{{ trans('messages.subscribers.resend_confirmation_email.confirm') }}" href="{{ route('SubscriberController@resendConfirmationEmail', $list->uid) }}">
									...
									<li>
										<a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ route('SubscriberController@delete', $list->uid) }}">
											<span class="material-symbols-rounded">delete_outline</span> {{ trans('messages.delete') }}
										</a>
									</li>
									<li>
										<a href="{{ route('SubscriberController@bulkDelete', $list->uid) }}"
	...
	<script>
        var SubscribersIndex = {
            getList: function() {
                return makeList({
                    url: '{{ route('Automation2Controller@subscribersList', $automation->uid) }}',
