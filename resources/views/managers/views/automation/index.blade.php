...
@section('page_header')
    ...
    <div class="page-title">
        <ul class="breadcrumb breadcrumb-caret position-right">
            <li class="breadcrumb-item"><a href="{{ action("HomeController@index") }}">{{ trans('messages.home') }}</a>
            </li>
        </ul>
        <h1>
...
@section('content')
    <div id="Automation2IndexContainer" class="listing-form"
         data-url="{{ route('Automation2Controller@listing') }}"
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item"
                                   ...
                                   href="{{ route('Automation2Controller@enable') }}"
                                >
                                    <span class="material-symbols-rounded me-2">play_arrow</span> {{ trans('messages.enable') }}
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" link-method="POST"
                                   link-confirm="{{ trans('messages.disable_automations_confirm') }}"
                                   href="{{ route('Automation2Controller@disable') }}">
                                    <span class="material-symbols-rounded me-2">hide_source</span> {{ trans('messages.disable') }}
                                </a></li>
                            <li>
                                <a class="dropdown-item" link-method='DELETE'
                                   link-confirm="{{ trans('messages.delete_automations_confirm') }}"
                                   href="{{ route('Automation2Controller@delete') }}">
...
<button type="button" class="btn btn-xs sort-direction" data-popup="tooltip"
        title="{{ trans('messages.change_sort_direction') }}" role="button" class="btn btn-xs">
                    ...
                    <span class="text-nowrap">
                        <input type="text" name="keyword" class="form-control search" value="{{ request()->keyword }}"
            ...
            <div class="text-end">
                <a href="{{ action("Automation2Controller@wizard") }}" role="button"
                   class="btn btn-secondary create-automation2">
    ...
    <script>
        var Automation2Index = {
            getList: function () {
                return makeList({
                    url: '{{ route('Automation2Controller@listing') }}',
